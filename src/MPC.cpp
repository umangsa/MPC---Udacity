 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (size_t t = 0; t < N - 1; t++) {
      ...
      //Speed and steering effect on each other
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  vector<double> actutor_values;
  actutor_values.push_back(solution.x[delta_start]);
  actutor_values.push_back(solution.x[a_start]);

  for (size_t i = 0; i < N-1; i++) {
    actutor_values.push_back(solution.x[x_start + i + 1]);
    actutor_values.push_back(solution.x[y_start + i + 1]);
  }
  return actutor_values;
